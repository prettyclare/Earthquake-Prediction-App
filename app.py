...
if st.button("🔍 Predict", type="primary", use_container_width=True):
    ...
    if pred == 1:
        st.error(
            "⚠️ **SIGNIFICANT** event predicted\n\n"
            "This event is predicted to be magnitude ≥ 5.0"
        )
    else:
        st.success(
            "✅ **Not significant** event predicted\n\n"
            "This event is predicted to be magnitude < 5.0"
        )
    ...
    with st.expander("See input features sent to model"):
        st.dataframe(
            build_input(latitude, longitude, depth, nst, gap, rms, dmin
